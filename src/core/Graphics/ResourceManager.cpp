...
void ResourceManager::draw(const sf::FloatRect& pos, const std::string& texture, const sf::Color& color)
{
    ...
    m_giga_buffer.append({ .position = {pos.position.x,                  pos.position.y},                .color = color, .texCoords = {uvleft,   uvtop} });
    m_giga_buffer.append({ .position = {pos.position.x + pos.size.x,     pos.position.y},                .color = color, .texCoords = {uvright,  uvtop} });
    m_giga_buffer.append({ .position = {pos.position.x + pos.size.x,     pos.position.y + pos.size.y},   .color = color, .texCoords = {uvright,  uvbottom} });
    m_giga_buffer.append({ .position = {pos.position.x,                  pos.position.y},                .color = color, .texCoords = {uvleft,   uvtop} });
    m_giga_buffer.append({ .position = {pos.position.x + pos.size.x,     pos.position.y + pos.size.y},   .color = color, .texCoords = {uvright,  uvbottom} });
    m_giga_buffer.append({ .position = {pos.position.x,                  pos.position.y + pos.size.y},   .color = color, .texCoords = {uvleft,   uvbottom} });
}
