...
Archer::Archer(int price) : Building(1000, 40, 1, 1000, price) {}
...
Tile* Archer::setTarget(std::mt19937& rand) {
  ...
  for (Path* p : m_tracked_path)
    if (p->hasEntity()) nonempty.push_back(p);
...
std::string Archer::projectileSkin() const { return "arrow"; }
