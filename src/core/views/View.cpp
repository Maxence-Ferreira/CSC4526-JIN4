...
View::View(ViewManager* vm, sf::RenderWindow* rw, std::string tileset, const std::unordered_map<std::string, std::shared_ptr<View>> &navigator, unsigned int seed) :m_context{
	.time = 0,
	.dt = 0,
	.offsetX = 0,
	.offsetY = 0,
...
},m_manager(vm), m_gui(*rw), m_gui_textures(), m_gui_widget(), m_navigator(navigator)
{
...
{
	if(ev->getIf<sf::Event::Resized>())for (const auto& [_, view] : m_navigator)
	{
		if(view)view->handle(ev);
	}
